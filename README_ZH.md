...
我用一个下午搭了自己的个人网站。一行代码没写 — 我只是告诉 AI 我想要什么样的，它就帮我全做了。然后免费上线。

这篇教程就是我的完整步骤。我能做到，你也一样可以。
...
| 工具 | 使用方式 | 价格 | 适合人群 |
|------|---------|------|---------|
| [Claude Code](https://docs.anthropic.com/en/docs/claude-code) | 在终端里对话，它帮你写代码 | 按量计费 | 不怕终端的人 |
...
2. **安装 Homebrew**（Mac 的软件包管理器，装东西用的）：
   ...
   ```
   按提示操作。如果要求输入 Mac 密码，直接打（屏幕不会显示字符，正常的）。如果 Homebrew 已经装过了，脚本会提示你 — 直接跳到下一步就行。
...
> **小贴士：** 如果项目文件夹里有不需要上传的文件（比如 `.DS_Store`），让 AI 助手帮你创建一个 `.gitignore` 文件。
...
> 如果 Git 要求输入用户名和密码（而不是弹出登录页面），告诉 AI 助手："帮我设置 GitHub 认证"，它会教你怎么做。
...
5. 保持默认设置，点 **"Deploy"**。如果 Vercel 问你"Framework Preset"，选 **Other** 或者留空。
6. 等十几秒，完成！
...
2. 输入你的域名 → 点 **Add**
3. Vercel 会显示你需要添加的 DNS 记录。直接复制它显示的值 — 每个项目的值不同。通常类似：
   - **A 记录：** `76.76.21.21`
